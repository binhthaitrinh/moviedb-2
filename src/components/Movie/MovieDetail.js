...
import { getDetail, getCredit, getTrailer, getReviews } from "../../actions/movie";
...
const MovieDetail = ({
  ...
  loading,
  getCredit,
  credits, reviewLoading, reviews,
  creditLoading, getTrailer, trailers, trailerLoading, getReviews
}) => {
  useEffect(() => {
        ...
        <div className="movie-details-body">
          ...
          <div className="movie-details-body-content">
            ...
            <div className="reviews">
              ...
              {reviewLoading ? null : reviews.map(review => (
                <div className="review">
                  <img src="https://via.placeholder.com/300" alt={review.author} />
                  <div className="review-details">
                    <h5 className="review-author">{review.author}</h5>
                    <p className="review-content">{review.content.split(' ').splice(0, 30).join(' ')}  <span>{review.content.split(' ').length >= 30 ? '...' : ''}</span> </p>
                    <a className="review-link" href={review.url} target="_blank">Read more >></a>
                  </div>
          ...
          <div className="movie-details-body-video">
            ...
            {trailerLoading ? null : trailers.map(trailer => (
              <>
                <iframe
                  title={trailer.name}
                  width="90%"
